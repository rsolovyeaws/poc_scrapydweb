...
Проект предоставляет две конфигурации Docker Compose для различных сценариев использования:

#### docker-compose.core.yml - Базовая конфигурация
**Рекомендуется для разработки и легковесного развертывания**

Включает только основные компоненты системы парсинга:
- ✅ Scrapyd (scrapyd1, scrapyd2, scrapydweb)
- ✅ Selenium Hub для браузерной автоматизации
- ✅ База данных (PostgreSQL, pgAdmin)
- ✅ Хранилище (MinIO, Redis)
- ✅ Очереди сообщений (RabbitMQ, task-processor)
- ✅ Балансировка нагрузки (NGINX, API Gateway)
- ✅ Прокси-сервисы (TinyProxy, proxy-rotator, ua-rotator)

**Исключены ресурсоемкие компоненты мониторинга:**
...
#### docker-compose.full.yml - Полная конфигурация
**Рекомендуется для продакшн-развертывания с полным мониторингом**

Включает все компоненты, включая полный стек мониторинга и логирования:
- ✅ Все компоненты из базовой конфигурации
...
**Используйте `docker-compose.core.yml` если:**
- Разрабатываете или тестируете систему
- Ограничены ресурсами сервера (CPU/RAM)
- Не требуется детальный мониторинг и логирование
- Хотите быстрый старт системы

**Используйте `docker-compose.full.yml` если:**
- Развертываете в продакшн-среде
- Требуется полный мониторинг метрик и централизованное логирование
- Необходимо отслеживать производительность и состояние системы
- Имеются достаточные ресурсы сервера
...
**Для базовой конфигурации (рекомендуется для начала):**
```bash
# Модифицируйте restart.sh для использования core конфигурации
docker-compose -f docker-compose.core.yml up -d --build
```

**Для полной конфигурации с мониторингом:**
```bash
# Используйте существующий скрипт restart.sh или
docker-compose -f docker-compose.full.yml up -d --build
```

**Или используйте оригинальный скрипт (запускает полную конфигурацию):**
```bash
./restart.sh
```
...
Сами задания можно отслеживать через стандартный интерфейс [ScrapydWeb](http://localhost:5000).
...
1. **Файловая система**: Логи сохраняются в директории `logs/` локально.
...
Осуществелена через Scrapydweb, в PoC в файле настройках настороено оповещение через Telegram об окончании работы паука. 
